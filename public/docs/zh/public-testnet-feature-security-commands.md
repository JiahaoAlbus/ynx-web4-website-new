...
- Bridge Service：`4/5` route 已 deposit-tested、`4/5` route 已观察到 release evidence，BTC/TRON 自动 watcher adapter 已在线，Sepolia release 仍 signer-gated，BSC lockbox 缺口仍未补齐；
- Web4 Hub：policy、session、agent、tool、intent、审计；
- AI Gateway：智能问答、任务、vault、payment、x402、链上 settlement；
- 本地服务器模型：Ollama；实时模型为 `qwen3:1.7b`，后台模型保留 `qwen3:4b/8b/14b`；官方事实类答案仍以实时链/RPC查询为准；
...
- AI 动作层：`/ai/actions` 和 `/ai/actions/run` 让 AI 能执行受控动作，例如查资产、查验证人、查 bridge readiness、创建监控任务、触发受保护的 bridge watcher scan；`trade.execute` 只有在 Web4 policy/session 和测试网 agent signer 都配置时才能提交交易。
